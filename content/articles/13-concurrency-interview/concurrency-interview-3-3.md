...
description: "Processes vs Threads - Concurrency Interview Module 3"
...
Cannot directly access other process's memory

Can access all memory in the process

Isolation

Strong (OS-enforced)

Weak (programmer responsibility)
...
Full process descriptor

Lightweight thread descriptor
...
AProcesses are created by the OS while threads are created by the application onlyBProcesses run on the CPU while threads run on the GPUCThreads can only run on a single CPU core while processes can run on multiple coresDProcesses have separate address spaces while threads within a process share the same address space
