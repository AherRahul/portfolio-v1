---
title: "Merkle Trees"
description: "Merkle Trees - System Design Module 15"
...
```shell
1Hash(Block1 + Block2 + Block3 + Block4) = abc123
```

Now we only need to compare one hash. But there's a problem: if the hashes don't match, we know _something_ is different, but we don't know _what_. We'd have to fall back to comparing everything.
