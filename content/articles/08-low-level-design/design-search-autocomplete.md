...
description: "Design a clean object model for a search autocomplete. Focus on responsibilities, relationships, and core flows."
...
*   For every character entered into the search box, a client sends a request to the backend for autocomplete suggestions. On average, 20 requests are sent for each search query. For example, the following 6 requests are sent to the backend by the time you finish typing “dinner”.
    ...
    *   `search?q=dinne`
    *   `search?q=dinner`
*   ~24,000 query per second (QPS) = 10,000,000 users _10 queries / day_ 20 characters / 24 hours / 3600 seconds.
*   Peak QPS = QPS \* 2 = ~48,000
*   Assume 20% of the daily queries are new. 10 million _10 queries / day_ 20 byte per query \* 20% = 0.4 GB. This means 0.4GB of new data is added to storage daily.
...
Let us use a simplified example to see how data gathering service works. Assume we have a frequency table that stores the query string and its frequency as shown in . In the beginning, the frequency table is empty. Later, users enter queries “twitch”, “twitter”, “twitter,” and “twillo” sequentially.  shows how the frequency table is updated.

![](https://books.dwf.dev/img/sd/sd-c14-f2.svg)
...
When a user types “tw” in the search box, the following top 5 searched queries are displayed, assuming the frequency table is based on Table 1.

![](https://books.dwf.dev/assets/images/sd-c14-f3-cf5a40435d1d441fc839dfcdb755ea3b.png)



To get top 5 frequently searched queries, execute the following SQL query:

![](https://books.dwf.dev/img/sd/sd-c14-f4.svg)
...
### Trie data structure[​](#trie-data-structure "Direct link to Trie data structure")
...
*   Each tree node represents a single word or a prefix string.
 ...
 shows a trie with search queries “tree”, “try”, “true”, “toy”, “wish”, “win”. Search queries are highlighted with a thicker border.

![](https://books.dwf.dev/img/sd/sd-c14-f5.svg)
...
After adding frequency info to nodes, updated trie data structure is shown in .

![](https://books.dwf.dev/img/sd/sd-c14-f6.svg)
...
Let us use an example as shown in  to explain the algorithm. Assume k equals to 2 and a user types “tr” in the search box. The algorithm works as follows:
...
![](https://books.dwf.dev/img/sd/sd-c14-f7.svg)
...
#### Limit the max length of a prefix[​](#limit-the-max-length-of-a-prefix "Direct link to Limit the max length of a prefix")
...
#### Cache top search queries at each node[​](#cache-top-search-queries-at-each-node "Direct link to Cache top search queries at each node")
...
By caching top search queries at every node, we significantly reduce the time complexity to retrieve the top 5 queries. However, this design requires a lot of space to store top queries at every node. Trading space for time is well worth it as fast response time is very important.
 ...
 shows the updated trie data structure. Top 5 queries are stored on each node. For example, the node with prefix “be” stores the following: \[best: 35, bet: 29, bee: 20, be: 15, beer: 10\].

![](https://books.dwf.dev/img/sd/sd-c14-f8.svg)
...
### Data gathering service[​](#data-gathering-service-1 "Direct link to Data gathering service")
...
Despite the differences in use cases, the underlying foundation for data gathering service remains the same because data used to build the trie is usually from analytics or logging services.
 ...
 shows the redesigned data gathering service. Each component is examined one by one.

![](https://books.dwf.dev/img/sd/sd-c14-f9.svg)
...
#### Aggregated Data.[​](#aggregated-data "Direct link to Aggregated Data.")
...
0 shows the mapping between the trie and hash table.

![](https://books.dwf.dev/img/sd/sd-c14-f10.svg)



In 0, each trie node on the left is mapped to the `<key, value>` pair on the right. If you are unclear how key-value stores work, refer to the "Design a key-value store" chapter.

### Query service[​](#query-service-1 "Direct link to Query service")

In the high-level design, query service calls the database directly to fetch the top 5 results. 1 shows the improved design as previous design is inefficient.

![](https://books.dwf.dev/assets/images/sd-c14-f11-c4cea1ba9144fc30cbc94e6a32b4248a.png)
...
*   Browser caching. For many applications, autocomplete search suggestions may not change much within a short time. Thus, autocomplete suggestions can be saved in browser cache to allow subsequent requests to get results from the cache directly. Google search engine uses the same cache mechanism. 2 shows the response header when you type “system design interview” on the Google search engine. As you can see, Google caches the results in the browser for 1 hour. Please note: “private” in cache-control means results are intended for a single user and must not be cached by a shared cache. “max-age=3600” means the cache is valid for 3600 seconds, aka, an hour.

![](https://books.dwf.dev/img/sd/sd-c14-f12.svg)
...
To mitigate the data imbalance problem, we analyze historical data distribution pattern and apply smarter sharding logic as shown in 5. The shard map manager maintains a lookup database for identifying where rows should be stored. For example, if there are a similar number of historical queries for 's' and for 'u', 'v', 'w', 'x', 'y' and 'z' combined, we can maintain two shards: one for 's' and one for 'u' to 'z'.

![](https://books.dwf.dev/img/sd/sd-c14-f15.svg)
