...
function handleViewOnSite() {
  ...
  window.open(`/learning/${fileName}`, '_blank')
}
</script>
  ...
  <div class="min-h-screen bg-black text-white">
      ...
      <div class="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center gap-4">
            ...
            <h1 class="text-xl font-bold">Manage Learning</h1>
          </div>
          <div class="flex items-center gap-4">
            <NuxtLink to="/learning/" target="_blank" class="text-zinc-400 hover:text-white">
      ...
      <div class="grid grid-cols-12 gap-6">
        <div class="col-span-12 lg:col-span-3">
          <button @click="creating = true" class="w-full bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md flex items-center justify-center gap-2 mb-4">
            <Icon name="heroicons:plus" />New Learning
          </button>
        ...
        <div class="col-span-12 lg:col-span-9">
          <div v-if="!isEditing" class="bg-zinc-900 border border-zinc-800 rounded-lg p-12 text-center">
            <Icon name="heroicons:light-bulb" class="text-6xl text-zinc-700 mx-auto mb-4" />
          ...
          <div v-else class="space-y-4">
            <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-4 flex items-center justify-between">
              <div class="flex items-center gap-3">
                <Icon name="heroicons:light-bulb" class="text-yellow-500 text-2xl" />
                <div>
    ...
    <div v-if="creating" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" @click.self="creating = false">
      <div class="bg-zinc-900 border border-zinc-800 rounded-lg p-6 max-w-md w-full">
        <h3 class="text-xl font-semibold mb-4">Create New Learning Resource</h3>
        <div class="mb-4">
          <label class="block text-sm font-medium text-zinc-300 mb-2">File Name</label>
          <input v-model="newFileName" type="text" placeholder="my-talk.md" class="w-full px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-red-500" @keyup.enter="createNewFile" />
          <p class="text-xs text-zinc-500 mt-1">Will be created in: {{ currentPath || 'learning/' }}</p>
        </div>
