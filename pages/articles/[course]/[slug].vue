...
// Construct the article path for nested structure
...
watchEffect(async () => {
  ...
  const doc = await queryContent('/courses').where({ _path: `/courses/${courseSlug}` }).findOne()
  courseDoc.value = doc
})
...
const currentLectureIndex = computed(() => {
  ...
  return courseLectures.value.findIndex(l => String(l._path) === currentSlug)
})
...
function goToNextTopic() {
  ...
  if (!n) return
  markComplete()
  router.push(`/articles/${course}/${n._path}`)
}

function goToPrevTopic() {
  const p = prevLecture.value
  if (!p) return
  router.push(`/articles/${course}/${p._path}`)
}
